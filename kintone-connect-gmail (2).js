...
(function (PLUGIN_ID, KC, GAPI) {
  ...
  var CONFIG_COMPANY_NAME = "_company_name_";
  var CONFIG_RECRUITER_NAME = "_recruiter_name_";
  var CONFIG_DEPARTMENT_NAME = "_department_name_";
  var CONFIG_REC_DEPART_NAME = "_recruiter_department_name_";
  var CONFIG_CALLER = "_company_caller_";
  var CONFIG_EMAIL_USER = "会社メールアドレス";
  ...
  kintoneMailService = {
      ...
      en: {
        ...
        message: {
          ...
          error: {
            ...
            unsupportExtension:
              'The "{{fileName}}" file is not supported to sending email. ' +
    ...
    setting: {
      ...
      i18n: {},
      app: {
        appClientID: "847478067421-a4u3d3r9rp7dgt8hal6o2n58806ef0gm.apps.googleusercontent.com",
        fieldsCode: {
          ...
          recruiterDepartmentName: "recruiter_department_name",
          departmentName: "department_name",
          companyCaller: "company_caller",
    ...
    kintoneEventListener: function (event) {
      ...
      KC.ui.element(this.data.ui.kintoneCustomizeGmailHeaderSigned).append(this.data.ui.btnSignOut);
    ...
    eventOnDetailShow: function (event) {
      ...
      this.data.ui.btnSendmail = KC.ui.createButton(
        this.setting.ui.buttons.sendMail,
        this.setting.i18n.button
      );
      KC.ui
        .element(this.data.ui.kintoneCustomizeGmailHeaderSigned)
    ...
    uiCreate: function (kintoneHeaderSpace) {
      ...
      this.data.ui.btnSignIn = KC.ui.createButton(
        this.setting.ui.buttons.signInGmail,
        this.setting.i18n.button
      );
      this.data.ui.btnSignOut = KC.ui.createButton(
        this.setting.ui.buttons.signOut,
        this.setting.i18n.button
      );
      this.data.ui.kintoneCustomizeGmailUserInfo = document.createElement("div");
      ...
      KC.ui
        .element(this.data.ui.kintoneCustomizeGmailHeaderNotSigned)
    ...
    updateSigninStatus: function (isSignedIn) {
      ...
      return self
        ...
        .then(function (userProfile) {
          ...
          self.data.ui.kintoneCustomizeGmailUserInfo.innerHTML =
            self.data.mail.profile.emailAddress;
        })
        .then(function () {
    ...
    initGetMail: function () {
      var self = this;
      KC.ui.loading.show();
      this.getMessageListFromGmail(self.setting.gAPI.labelIdsForGet)
        .then(function (emailMessages) {
    ...
    putMailToKintoneApp: function () {
      ...
      return self.getExitsMessageIdsOnkintone().then(function (kintoneMessageRecordIds) {
        var messageInfos = self.data.emailMessages
    ...
    removeStyleTagOnString: function (text) {
      return text.replace(/<style.*style>|<style.*[[\s\S\t]*?.*style>/gm, "");
    },
    ...
    addMailIntoKintone: function (mailInfo, headers, body, files) {
      ...
      if (typeof files !== "undefined" && files.length > 0) {
        return this.uploadFileToKintone(mailInfo.messageID, files, 0, [])
          .then(function (filesKey) {
    ...
    uploadFileToKintone: function (messageID, files, index, kintoneFilesKey) {
      var self = this,
        file = files[index];
      return this.gmail("me")
        .getAttachmentByID(messageID, file.body.attachmentId)
    ...
    initSendMail: function (recordID) {
      ...
      return this.kintoneGetRecordData(recordID)
        ...
        .then(function (dataEmail) {
          ...
          if (
            typeof dataEmail.attachment !== "undefined" &&
            dataEmail.attachment.constructor === Array &&
            dataEmail.attachment.length > 0
          ) {
            hasAttachment = true;
            dataEmail.attachment.forEach(function (att) {
              ...
              if (self.setting.extensionProhibited.indexOf(fileExt) >= 0) {
                self.stopProcessWithMessage(
                  self.setting.i18n.message.error.unsupportExtension
          ...
          if (!self.isValidFileSize(mineEmailByte)) {
            self.stopProcessWithMessage(
              self.setting.i18n.message.error.mailSizeLarge.replace(
                "{{maxFileSize}}",
                self.convertByteSize(self.setting.gAPI.maxEmailSize, "MB")
              )
            );
          }
          ...
          return self.kintoneGetAttachments(dataEmail.attachment).then(function (mailAttachments) {
            mailAttachments.forEach(function (att) {
              mineEmail = mineEmail.replace(
                '"' + att.fileKey + '"',
                '"' + att.name + '"\r\n\r\n' + att.contentBytes + "\r\n\r\n"
              );
            });
    ...
    parseRecordDataToEmailData: function (recordData) {
      ...
      return new kintone.Promise(function (resolve, reject) {
        return kintone.api(url, "GET", { app: 638, id: 5 }, function (response) {
          var attachmentDefault = response.record.attachment.value;

          var result = {};
          for (var key in mySelf.setting.app.fieldsCode) {
            var fieldCode = mySelf.setting.app.fieldsCode[key];
          ...
          var contentMail = result["content"];
          var companyName = result["companyName"];
          var recruiterName = result["recruiterName"];
          var recruiterDepartmentName = result["recruiterDepartmentName"];
          var departmentName = result["departmentName"];
          var companyCaller = result["companyCaller"];

          contentMail = contentMail.replace(CONFIG_RECRUITER_NAME, recruiterName);
          contentMail = contentMail.replace(CONFIG_REC_DEPART_NAME, recruiterDepartmentName);
          contentMail = contentMail.replace(CONFIG_COMPANY_NAME, companyName);
          contentMail = contentMail.replace(CONFIG_DEPARTMENT_NAME, departmentName);
          contentMail = contentMail.replace(CONFIG_CALLER, companyCaller);

          result["content"] = contentMail;

          result["attachment"] = attachmentDefault;

          var selectEmail = result["selectEmail"];
          if (selectEmail == CONFIG_EMAIL_USER) {
            result["to"] = recordData["to"].value;
            // result["bcc"] = recordData["bcc"].value;
          } else {
            result["to"] = recordData["bcc"].value;
            // result["bcc"] = recordData["to"].value;
          }
          // Force set From sender with value is the email of current user-login (gmail)
          result.from = mySelf.data.mail.profile.emailAddress;
          resolve(result);
        });
      }).then(
        function (result) {
    ...
    kintoneGetAttachments: function (attachments) {
      ...
      return kintone.Promise.all(attachmentPromise).catch(function (error) {
        KC.showError(error);
      });
    },
    ...
    mineMessage: function (data) {
      ...
      if (
        typeof data.attachment !== "undefined" &&
        data.attachment.constructor === Array &&
    ...
    encodeSender: function (senderString) {
      ...
      senderArray.forEach(function (sender) {
        ...
        if (!this.isValidEmail(email)) {
          this.stopProcessWithMessage(
            self.setting.i18n.message.error.invalidEmail + " (" + email + ")"
          );
        }
